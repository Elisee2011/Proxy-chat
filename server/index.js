...
const PORT=Number(process.env.PORT||3001);const app=express();app.use(express.json({limit:'8mb'}));const players=new Map();const media=new Map();const CHAT_RADIUS=1500;
app.get('/health',(_,res)=>res.json({ok:true,service:'proxy-chat',players:players.size,media:media.size}));
app.get('/media',(_,res)=>res.json([...media.values()].map(({id,ownerId,ownerName,...m})=>({id,ownerId,ownerName,...m}))));
app.post('/media',(req,res)=>{const d=req.body||{};if(!['image','video','youtube'].includes(d.type))return res.status(400).json({error:'Type de média invalide'});const url=String(d.src||'');if(!url||url.length>7000000)return res.status(413).json({error:'Média trop volumineux'});if(d.type==='youtube'&&!/^[\w-]{6,}$/.test(url))return res.status(400).json({error:'Identifiant YouTube invalide'});const item={id:String(d.id||crypto.randomUUID()),ownerId:String(d.ownerId||'anonymous'),ownerName:String(d.ownerName||'Player').slice(0,18),type:d.type,src:url,x:Math.max(0,Math.min(2400,Number(d.x)||0)),y:Math.max(0,Math.min(1500,Number(d.y)||0)),width:Math.max(40,Math.min(1000,Number(d.width)||400)),height:Math.max(40,Math.min(800,Number(d.height)||240)),rotation:Number(d.rotation)||0,autoplay:Boolean(d.autoplay),loop:Boolean(d.loop)};media.set(item.id,item);broadcastMedia({type:'media:published',media:item});res.json(item)});
