...
wss.on('connection',ws=>{let id=null;
 ws.on('message',raw=>{let data;try{data=JSON.parse(raw.toString());}catch{return;}
  ...
  if(data.type==='move'){me.x=Math.max(0,Math.min(2400,Number(data.x)||me.x));me.y=Math.max(0,Math.min(1500,Number(data.y)||me.y));broadcast({type:'player:move',player:publicPlayer(me)},id);}
