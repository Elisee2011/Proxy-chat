...
function broadcastAllPositions(){for(const [id] of players){for(const [otherId,other] of players){if(id!==otherId)broadcast({type:'player:move',player:publicPlayer(other)},id);}}}
...
// Réémet les positions à 10 Hz. Le client sait déjà traiter "player:move".
setInterval(()=>{if(players.size>1)broadcastAllPositions();},100);
